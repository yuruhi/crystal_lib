...
data:
  ...
  bundledCode: "# require \"./graph\"\nstruct Edge(T)\n  include Comparable(Edge(T))\n\
    ...
    \ end\n    result.reverse\n  end\nend\n"
  code: "require \"./graph\"\n\nclass UnweightedGraph\n  private def topological_sort_dfs(result,\
    \ visited, v)\n    unless visited[v]\n      visited[v] = true\n      graph[v].each\
  ...
  verifiedWith:
  - test/topological_sort.test.cr
documentation_of: graph/topological_sort.cr
layout: document
redirect_from:
- /library/graph/topological_sort.cr
- /library/graph/topological_sort.cr.html
