...
data:
  ...
  bundledCode: "# verification-helper: PROBLEM https://onlinejudge.u-aizu.ac.jp/problems/2885\n\
    ...
    \ = {false, true}.map { |c| color.count(c) }.select(&.even?).map(&.//(2)).uniq!.sort!\n\
    \    puts ans.size\n    puts ans.join('\\n') unless ans.empty?\n  else\n    puts\
    \ 0\n  end\nend\n"
  ...
  verifiedWith: []
documentation_of: test/graph/bipartite_graph.test.cr
layout: document
redirect_from:
- /verify/test/graph/bipartite_graph.test.cr
- /verify/test/graph/bipartite_graph.test.cr.html
