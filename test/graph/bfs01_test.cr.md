...
data:
  ...
  bundledCode: "# verification-helper: PROBLEM https://yukicoder.me/problems/no/1607\n\
    ...
    \ start < size\n    queue = Deque{start}\n    dist = Array(Int32?).new(size, nil).tap(&.[start]\
    \ = 0)\n    while v = queue.shift?\n      current_dist = dist[v].not_nil!\n  \
    ...
    \ goal, &.cost)\n  end\n\n  # Returns the distance from *start* to *goal* if path\
    \ is present, otherwise raises `ArgumentError`.\n  # *block* must return `0` or\
    \ `1`.\n  def bfs01_st!(start : Int, goal : Int, &block) : Int32\n    bfs01_st(start,\
    \ goal) { |edge| yield edge } || raise ArgumentError.new\n  end\n\n  # Returns\
    \ the distance from *start* to *goal* if path is present, otherwise raises `ArgumentError`.\n\
    \  # All of cost of edge must be `0` or `1`.\n  def bfs01_st!(start : Int, goal\
    ...
    puts (0..2*10**5).bsearch { |x|\n  d1 = graph.bfs01_st(0, n - 1) { |edge| edge.cost\
    \ > x ? 1 : 0 }.not_nil!\n  d2 = graph.bfs01_st!(0, n - 1) { |edge| edge.cost\
  ...
  code: "# verification-helper: PROBLEM https://yukicoder.me/problems/no/1607\nrequire\
    ...
    \ { |edge| edge.cost > x ? 1 : 0 }.last.not_nil!\n  d4 = graph.bfs01!(0) { |edge|\
    \ edge.cost > x ? 1 : 0 }.last\n  raise \"\" unless d1 == d2 == d3 == d4\n  d1\
    \ < k\n}\n"
  dependsOn:
  - src/graph/bfs01.cr
  - src/graph.cr
