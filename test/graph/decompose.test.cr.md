...
data:
  ...
  bundledCode: "# verification-helper: PROBLEM https://yukicoder.me/problems/no/922\n\
    ...
    \ { |(edge, i)| edge.to != p }.reduce(T.new) { |acc, (edge, i)|\n      acc + (@dp[v][i]\
    ...
    \ j|\n        index[v] = {i, j}\n      end\n    end\n\n    graphs = Array.new(groups.size)\
    \ { |i| self.class.new(groups[i].size) }\n    if self.class.directed?\n      each\
    ...
    end\n\nn, m, q = input(i, i, i)\ng = UnweightedUndirectedGraph.new n, input({i\
    \ - 1, i - 1}[m])\n\ngraphs, index = g.decompose\nlcas = graphs.map { |graph|\
    \ LCA.new(graph, 0) }\n\nans = 0i64\n\ncnts = Array.new(graphs.size) { |i| Array.new(graphs[i].size,\
    \ 0) }\nq.times do\n  a, b = input(i - 1, i - 1)\n  ai, aj = index[a]\n  bi, bj\
  ...
  requiredBy: []
  timestamp: '2021-08-02 14:13:56+09:00'
  verificationStatus: TEST_ACCEPTED
  verifiedWith: []
documentation_of: test/graph/decompose.test.cr
layout: document
redirect_from:
- /verify/test/graph/decompose.test.cr
- /verify/test/graph/decompose.test.cr.html
