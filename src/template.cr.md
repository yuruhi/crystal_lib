...
data:
  ...
  attributes: {}
  bundledCode: "# require \"./comparable/min_max\"\nmodule Comparable(T)\n  def min(x\
    \ : T)\n    self > x ? x : self\n  end\n\n  def max(x : T)\n    self < x ? x :\
    ...
    # require \"./array/new\"\nclass Array(T)\n  def self.new(sizes : Tuple(*I), initial_value)\
    ...
    \    {% for i in 0...I.size %} } {% end %}\n    {% end %}\n  end\nend\n\n# require\
    \ \"./collection/accumulate\"\nmodule Enumerable(T)\n  def accumulate(init : U)\
    \ : Array(U) forall U\n    accumulate(init) { |a, b| a + b }\n  end\n\n  def accumulate\
    \ : Array(T)\n    accumulate { |x, y| x + y }\n  end\n\n  def accumulate(init\
    \ : U, &block : U, T -> U) : Array(U) forall U\n    result = [init]\n    each\
    \ { |v| result << yield(result.last, v) }\n    result\n  end\n\n  def accumulate(&block\
  ...
  code: 'require "./comparable/min_max"
